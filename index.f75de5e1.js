...
tbody.addEventListener("click", (e)=>{
    if (e.target.tagName === "TD") e.target.parentElement.classList.toggle("active");
});
tbody.addEventListener("dblclick", (e)=>{
    if (e.target.tagName === "TD") {
        ...
        input.addEventListener("keydown", function(ev) {
            if (ev.key === "Enter") cell.textContent = input.value || originalText;
        });
...
formBtn.addEventListener("click", (e)=>{
    ...
    const formSalaryValue = !formValues.salary || isNaN(formValues.salary) ? "" : "$" + addSeparator(+formValues.salary);
...
function validateFormFields({ employeeName, position, office, age, salary }) {
    const notifications = [];
    if (employeeName.length < 4) notifications.push(new CustomNotification("Error", "Name must be at least 4 characters long", "error"));
    if (!position.trim()) notifications.push(new CustomNotification("Error", "Position cannot be empty", "error"));
    if (!office.trim() || !offices.includes(office)) notifications.push(new CustomNotification("Error", "Office cannot be empty, and should contain one of the defined options", "error"));
    if (age < 18 || age > 90) notifications.push(new CustomNotification("Error", "Age must be between 18 and 90", "error"));
    if (salary <= 0 || isNaN(salary)) notifications.push(new CustomNotification("Error", "The value of salary must be a number greater than 0", "error"));
    if (notifications.length === 0) notifications.push(new CustomNotification("Success", "Data successfully added", "success"));
...
headers.forEach((header)=>{
    header.addEventListener("click", ()=>{
        ...
        const newSortOrder = !currentSort || currentSort === "DESC" ? "ASC" : "DESC";
    ...
    switch(val){
        ...
        default:
            return arr.sort((a, b)=>{
                ...
                return order === "DESC" ? rowB.localeCompare(rowA) : rowA.localeCompare(rowB);
            });
    }
}
